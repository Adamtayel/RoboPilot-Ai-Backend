...
/**
 * Locates likely product-listing anchors (same "product" heuristic as
 * extractBestMatch) and returns a handful of SHORT text windows around each
 * one, instead of the full page. Two benefits: (1) token cost to DeepSeek
 * drops roughly 5-10x since we're not paying for nav menus/headers/footers,
 * and (2) accuracy improves because the model is looking at focused,
 * likely-relevant regions instead of getting lost in a large page. Returns
 * "" (no DeepSeek call made) when the page has no product-like anchors at
 * all — saves a wasted API call outright.
 */
export function buildCandidateSnippets(html: string): string {
  ...
  while ((match = anchorRegex.exec(html)) !== null && snippets.length < MAX_CANDIDATE_SNIPPETS) {
    const attrs = match[1];
    if (!attrs || !/product/i.test(attrs)) continue;
...
cat > src/lib/robopilot/deepseek-extractor.ts << 'FILE_EOF_2'
/**
...
cat > tests/unit/live-pricing.test.ts << 'FILE_EOF_3'
...
echo "Done. Run: npm run typecheck && npm test (expect 57 passed)"
